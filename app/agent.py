...
class Agent:
    ...
    async def _process_openai(self, prompt: str, business_id: int, token: str = None, client_id: str = None, biz_hash: str = None) -> Dict[str, Any]:
        ...
        system_prompt = (
            ...
            "If exactly one service is found, YOU MUST IMMEDIATELY proceed to `create_appointment` with that ID. DO NOT ask the user for confirmation if there is only one match. "
    ...
    async def _process_gemini(self, prompt: str, business_id: int, token: str = None, client_id: str = None, biz_hash: str = None) -> Dict[str, Any]:
        ...
        while True:
            ...
            # Optimization: If list_offers was called, return immediately without sending back to LLM
            # The tool result (WhatsApp text) is sufficient.
            if any(fc.name == "list_offers" for fc in function_calls):
                logger.info("Optimization: list_offers called, skipping final LLM generation")
                break
